...
public class While_Macro_Controller extends Controller {
    ...
    private ArrayList<ParseTree> whileslist = new ArrayList<>();            // enthält alle while context in rogram
    ...
    private ParseTree whilesContext  ;          // ist der Context für den haupt while des programm
    ...
    public While_Macro_Controller(String[] args) {
        super(args);
        this.args = args;
    }
    ...
    public void speicherungElemente(ParseTree tree) {          // holt und speichert die Eingaben
        ...
        While_Macro_Model model = new While_Macro_Model(tree);
    ...
    public void toJavaCode() {
        ...
        if (unique) { // wenn der Name jedes macos ist einzigartig
            ...
            for (int i = 0; i < whileslist.size() ; i++) {

                While_Controller controller = new While_Controller(args);
                controller.setIs_haupt_while(false);
                ...
                controller.pos_Macro = parameters_Anzahl.get(i);                Helper_verifier prufer = new Helper_verifier();
                controller.speicherungElemente(whileslist.get(i));
                ...
                if(isGultig){  // wenn alle aufgerufenen Parameter gulig sind
                    ...
                    if (pruf ){  // wenn aalle macro anruf gultig sind
        ...
        if (unique){
            if (! hat_fehler){
                ...
                While_Controller controller = new While_Controller(args);
                controller.macro_exist = true;
                controller.speicherungElemente(whilesContext);
    ...
    private String tojavaCode_macro (){   // erzeugung des Javacode für ein Macro
        ...
        for (int i = allKomponentercounter; i < allKomponente.size() ; i++) {
            ...
            if (text.equals("MACRO")){   // wenn es geht um ein Macro Schlüssel
