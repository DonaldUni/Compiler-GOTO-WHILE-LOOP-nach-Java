...
public class Loop_Macro_Controller extends Controller {
    ...
    public Loop_Macro_Controller(String[] args) {
        super(args);
        this.args = args;
    }
    ...
    public void speicherungElemente(ParseTree tree) { // holt und speichert die Eingaben
        ...
        Loop_Macro_Model  model = new Loop_Macro_Model(tree);
    ...
    public void toJavaCode() {
        ...
        if ( unique) { // wenn der Name jedes macos und parameter einzigartig ist
            int positiom_start = 0;
            for (int i = 0; i < loopslist.size() ; i++) {   // für alle loop programm, der zu einem Macro gehört , erzeugen wir den Java Code

                Loop_Controller controller = new Loop_Controller(args);
                controller.setIs_haupt_loop(false);
                controller.speicherungElemente(loopslist.get(i));
        ...
        if (unique){
            if (! hat_fehler){
                ...
                controller.speicherungElemente(loopsContext);
