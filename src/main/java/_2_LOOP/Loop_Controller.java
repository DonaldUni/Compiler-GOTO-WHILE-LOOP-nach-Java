...
public class Loop_Controller extends Controller {
    // speichert die Eingaben durch den Model , Compielert die in JavaCode String und zeigt den fertigen JavaCode an
    ...
    private boolean is_haupt_loop = true;
    protected int pos_Macro = 0;
    protected int positiom_start = 0;



    public Loop_Controller(String[] args) {
        super(args);
        this.args = args;
    }
    ...
    public void speicherungElemente(ParseTree tree) {
        ...
        Loop_Model loop_model = new Loop_Model(tree);  // erzeugt eine Instance von Loop_Model um ihre Daten zu nutzen

        // deep Copy of map von Model , von Parameter, Variables_name  und die andere nötigen Elemente von loopModel
        map.putAll(loop_model.getMap());
        parameters_names.addAll(loop_model.getParameter_names());
        variables_names.addAll(loop_model.getVariables_names());
        macro_anruf.addAll(loop_model.getMacro_anruf());
        all_macro_Name_anruf.addAll(loop_model.getAll_macro_Name_anruf());
        macro_param_name.addAll(loop_model.getMacro_param_name());
        macro_anzahl_param_name.addAll(loop_model.getMacro_anzahl_param_name_());

        // eingabeMethod()  fur parameters und variables
        copyValue(args);

        if (is_haupt_loop){ // wenn es geht um den HauptLoop , prufman besonders die Prameter in macro anruf
    ...
    public void toJavaCode() {
        ...
        for (int i = 0; i < map.size() ; i++) {
            ...
            // Falls es ein Loop ist
            if (map.get(i).equals("LOOP")){

                javaCode = tabulationMaker(javaCode);              // Tabulation einfügen um die Strukture des Codes zu verschönen

                String count = countGenerator();
                String counterName = countNameGenerator();

                javaCode = javaCode + "int " + counterName + " = " + map.get(i+1)+ ";" + "\n";

                javaCode = tabulationMaker(javaCode);              // Tabulation einfügen um die Strukture des Codes zu verschönen
                javaCode = javaCode + "for( int " + count + " = 0; " + count + " < " + counterName + "; " + count + "++ ){" +"\n" ;
            ...
            if (!(map.get(i).equals("LOOP")||map.get(i).equals("DO")||map.get(i).equals("END")||map.get(i).equals("IF")||map.get(i).equals("THEN") ||map.get(i).equals(";"))){
    ...
    // Erzeugt die Zähler i für for Schleife: Bsp : hier "i0",  for ( int i0 = 0; i0 < Counter ; i++) {}
    private  String countGenerator (){

        String count = "i" + counterAnzahl;
        ++counterAnzahl;

        if (variables_names.contains(count)){  // prüft ob es kein Konflikt mit Einem von Variablenamen, die vom Nutzer eingegeben wurden.
    ...
    // Erzeugt die Zähler counter für for Schleife: Bsp : hier "counter0",  for ( int i0 = 0; i0 < counter0 ; i++) {}
    private  String countNameGenerator (){

        String counterName = "counter" + counterAnzahl1;
        ++counterAnzahl1 ;

        if ( variables_names.contains(counterName)){   // prüft ob es kein Konflikt mit Einem von Variablenamen, die vom Nutzer eingegeben wurden.
            counterName = countNameGenerator();
        }

        return counterName;
    }

    //Eingabemethode für parameter und Variable
    ...
    // pruf ob es die anweisung eine "-" enthätlt und fugt  ein if bedingung in diesem Fall hinzu
    ...
    protected void setIs_haupt_loop(boolean is_haupt_loop) {
        this.is_haupt_loop = is_haupt_loop;
    }
